...
if (isset($_POST['submit_task_action'])) {
    ...
    $taskStatus = $taskObj->getTaskStatus($taskId); // Assuming method to get task status

    // Debugging: Check current task status
    echo "Current task status: " . $taskStatus . "<br>";

    // Avoid updating the task if it's already completed
    if ($taskStatus != 'completed') {
        // Debugging: Check if the status is being updated
        echo "Updating task status to completed...<br>";

        $taskObj->updateTaskStatus($taskId, 'completed');  // Set task status to 'completed'

        // Check if update was successful
        $updatedStatus = $taskObj->getTaskStatus($taskId);
        echo "Updated task status: " . $updatedStatus . "<br>";
        ...
        if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($uploadSuccess) {
                echo "File uploaded successfully and task marked as completed!<br>";
            } else {
        ...
        if (isset($_POST['report_content']) && !empty($_POST['report_content'])) {
            ...
            if ($reportSuccess) {
                echo "Report generated and saved successfully! Task marked as completed!<br>";
            } else {
                echo "Error generating the report.<br>";
            }
        }
    } else {
        echo "Task is already completed.<br>";
    }
