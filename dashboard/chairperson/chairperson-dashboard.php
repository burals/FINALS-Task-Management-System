<?php
    ...
    // Fetch profile picture path, default to a placeholder if not set
    $profilePicturePath = "../uploads/" . $user_data['id'] . "/profile.jpg";
    if (!file_exists($profilePicturePath)) {
        $profilePicturePath = "../../src/images/default-profile.jpg"; // Set a default image if no profile picture exists
    }
    ...
    $recent_tasks_stmt = $admin->runQuery("SELECT title, description, status, updated_at FROM tasks ORDER BY updated_at DESC LIMIT 5");
    $recent_tasks_stmt->execute();
    $recent_tasks = $recent_tasks_stmt->fetchAll(PDO::FETCH_ASSOC);

    $pending_tasks_stmt = $admin->runQuery("SELECT COUNT(*) AS pending_count FROM tasks WHERE status = 'pending'");
